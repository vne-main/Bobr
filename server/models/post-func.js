...
class Place {
  ...
  static async getPost(objData) {
    const findPost = await posts.findById({ _id: objData._id }).catch(() => {
      return 500;
    });
    ...
    if (!checkIp) {
      ...
      return await posts.findById({ _id: objData._id }).catch(() => {
        return 500;
      });
    } else {
      return await posts.findById({ _id: objData._id }).catch(() => {
  ...
  static async addComment(newComment) {
    ...
    await posts.update(
      { _id: newComment._id },
      { $push: { comments: objComment } }
    );
    return await posts.findById({ _id: newComment._id }).catch(() => {
      return 500;
    });
  }
}
